...
namespace MarkovJunior
{
    ...
    static class Graphics
    {
        ...
        /// <see cref="RGB(int)">Graphics.RGB</see> to unpack the
        /// (r, g, b) values.
        /// </summary>
        ...
        public static (int[], int, int, int) LoadBitmap(string filename)
        {
            try
            {
                string name = "Assets/Resources/MarkovJunior/" + filename;
                Texture2D t = LoadPNG(name);
                int width = t.width;
                int height = t.height;
                UnityEngine.Color[] cList = t.GetPixels();
                int[] result = new int[width * height];
                for (int i = 0; i < height; i++)
                {
                    for (int j = 0; j < width; j++)
                    {
                        UnityEngine.Color c = cList[i * width + j];
                        string s = ColorUtility.ToHtmlStringRGBA(c);

                        result[(height - i - 1) * width + j] = int.Parse(s, System.Globalization.NumberStyles.HexNumber);
                        // Debug.Log(s);
                    }
                }
        ...
        /// ints; call <see cref="Int(byte, byte, byte)">Graphics.Int</see>
        ...
        /// </summary>
        /// <returns><inheritdoc cref="Render(byte[], int, int, int, int[], int, int)" path="/returns"/></returns>
        public static (int[], int, int) BitmapRender(byte[] state, int MX, int MY, int[] colors, int pixelsize, int MARGIN)
        {
            ...
            for (int i = 0; i < bitmap.Length; i++) bitmap[i] = MarkovJunior.GUI.BACKGROUND;
        ...
        static readonly Dictionary<int, MarkovSprite> sprites = new();
        ...
        public static (int[], int, int) IsometricRender(byte[] state, int MX, int MY, int MZ, int[] colors, int blocksize, int MARGIN)
        {
            ...
            for (int i = 0; i < screen.Length; i++) screen[i] = MarkovJunior.GUI.BACKGROUND;
                ...
                for (int dy = 0; dy < SY; dy++) for (int dx = 0; dx < SX; dx++)
                    {
                        ...
                        byte R = (byte)(r * (float)grayscale / 256.0f);
                        byte G = (byte)(g * (float)grayscale / 256.0f);
                        byte B = (byte)(b * (float)grayscale / 256.0f);
                        int X = x + dx;
                        int Y = y + dy;
            ...
            bool success = sprites.TryGetValue(blocksize, out MarkovSprite sprite);
            if (!success)
            {
                sprite = new MarkovSprite(blocksize);
