...
namespace MarkovJunior
{
    ...
    class Interpreter
    {
        ...
        /// at the start of the program; rather, the <see cref="first">Interpreter.first</see>
        /// list holds indices into this list.
        /// </summary>
        public List<(int, int, int)> changes;

        /// <summary>
        /// A list of indices into the <see cref="changes">Interpreter.changes</see>
        ...
        public static Interpreter Load(XElement xelem, int MX, int MY, int MZ)
        {
            ...
            if (ip.grid == null)
            {
                UnityEngine.Debug.LogWarning("failed to load grid");
                return null;
            }
            ...
            if (symmetry == null)
            {
                UnityEngine.Debug.LogWarning($"unknown symmetry {symmetryString} at line {xelem.LineNumber()}");
                return null;
            }
        ...
        public IEnumerable<(byte[], char[], int, int, int)> Run(int seed, int steps, bool gif)
        {
            ...
            while (current != null && (steps <= 0 || counter < steps))
            {
                if (gif)
                {
                    UnityEngine.Debug.Log($"Step : [{counter}]");
        ...
        public static void WriteLine(string s) => UnityEngine.Debug.Log(s);
