            ...
            try
            {
                ...
                while (stream.BaseStream.Position < stream.BaseStream.Length)
                {
                    ...
                    if (head == 'S')
                    {
                        ...
                        stream.ReadBytes(4);
                        // Interpreter.WriteLine("found SIZE chunk");
                        MX = stream.ReadInt32();
                        MY = stream.ReadInt32();
                        MZ = stream.ReadInt32();
                        stream.ReadBytes(chunkSize - 4 * 3);
                        // Interpreter.WriteLine($"size = ({MX}, {MY}, {MZ})");
                    }
                    else if (head == 'X')
                    {
                        string tail = Encoding.ASCII.GetString(stream.ReadBytes(3));
                        ...
                        // Interpreter.WriteLine("found XYZI chunk");
                        stream.ReadBytes(8);
                        int numVoxels = stream.ReadInt32();
                        // Interpreter.WriteLine($"number of voxels = {numVoxels}");
                        for (int i = 0; i < numVoxels; i++)
                        {
                            ...
                            // Interpreter.WriteLine($"adding voxel {x} {y} {z} of color {color}");
                        }
                    }
                }
                file.Close();
