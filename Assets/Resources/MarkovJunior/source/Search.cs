...
namespace MarkovJunior
{
    ...
    static class Search
    {
        ...
        public static byte[][] Run(byte[] present, int[] future, Rule[] rules, int MX, int MY, int MZ, int C, bool all, int limit, double depthCoefficient, int seed)
        {
            ...
            MinMaxHeap<BoardPair> frontier = new(new List<BoardPair>(), new BoardComparer());
            Random random = new(seed);

            frontier.Add(new BoardPair(0, rootBoard.Rank(random, depthCoefficient)));
            ...
            {
                BoardPair b = frontier.ExtractMin();
                int parentIndex = b.index;
                frontierLength--;
                ...
                //for (int c = 0; c < children.Length; c++)
                {
                    ...
                    if (success)
                    {
                        ...
                        if (parentBoard.depth + 1 < oldBoard.depth)
                        {
                            ...
                            {
                                frontier.Add(new BoardPair(childIndex, oldBoard.Rank(random, depthCoefficient)));
                                frontierLength++;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            frontier.Add(new BoardPair(childIndex, childBoard.Rank(random, depthCoefficient)));
                            frontierLength++;
                        }
                    }
                }
        ...
        static List<byte[]> OneChildStates(this byte[] state, int MX, int MY, Rule[] rules)
        {
            ...
            foreach (Rule rule in rules)
                for (int y = 0; y < MY; y++) for (int x = 0; x < MX; x++)
                        if (rule.Matches(x, y, state, MX, MY)) result.Add(Applied(rule, x, y, state, MX));
            return result;
        }
        ...
        static byte[] Apply(this byte[] state, List<(Rule, int)> solution, int MX)
        {
            byte[] result = new byte[state.Length];
            Array.Copy(state, result, state.Length);
            foreach (var (rule, i) in solution) rule.Apply(i % MX, i / MX, result, MX);
            return result;
        }
    }
    ...
    class Board
    {
        ...
        /// <param name="parentIndex"><inheritdoc cref="parentIndex" path="/summary"/></param>
        /// <param name="depth"><inheritdoc cref="depth" path="/summary"/></param>
        /// <param name="backwardEstimate"><inheritdoc cref="backwardEstimate" path="/summary"/></param>
        /// <param name="forwardEstimate"><inheritdoc cref="forwardEstimate" path="/summary"/></param>
